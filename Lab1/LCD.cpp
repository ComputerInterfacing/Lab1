...
void LCD::Invert(void) {
	...
	if (inverted) {
		send_cmd(CMD_FUNCTION_SET |OPT_BASIC_INSTR); // We must send this before modifying the display control
		send_cmd(CMD_DISPLAY_CONTROL |OPT_NORMAL); // Set display control to normal mode

		inverted = false;
	} else {

		send_cmd(CMD_FUNCTION_SET |OPT_BASIC_INSTR); // We must send this before modifying the display control
		send_cmd(CMD_DISPLAY_CONTROL |OPT_INVERSE); // Set display control to inverse mode
...
void LCD::init_lcd(void) {
	...
	// H = 1
	send_cmd(CMD_FUNCTION_SET |OPT_EXT_INSTR); // Tell LCD extended Commands follow
 ...
 */
void LCD::move(point loc) {
	send_cmd(CMD_FUNCTION_SET |OPT_BASIC_INSTR);
	send_cmd(0x40+loc.row);//move x
	send_cmd(0x80+loc.col);// move y
}
